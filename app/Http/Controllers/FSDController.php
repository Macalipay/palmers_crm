...
class FSDController extends Controller
{
    public function index() {
       ...
       if (Auth::user()->designation == 'SUPER ADMIN' || Auth::user()->designation == 'Super Admin') {
        ...
        $supervisor = Personnel::where('position', 'Design & Supervisor')->where('type', 2)->get();
        $design = Personnel::where('position', 'Design & Estimate')->where('type', 2)->get();
        $sales = Personnel::where('position', 'Sales In Charge')->where('type', 2)->get();

        return view('backend.pages.sales.fsd', compact('sources', 'merchandisers', 'sales_agents', 'sales_associates', 'divisions', 'branches', 'companies', 'items', 'brands',
                                                        'overall_transaction', 'this_year_transaction', 'today_transaction', 'supervisor', 'design', 'sales'));
    }

    public function userDashboard() {
        return view ('backend.pages.dashboard.fsd');
    }
    
    public function get() {
        if(Auth::user()->designation == 'SUPER ADMIN' || Auth::user()->designation == 'Super Admin') {
            if(request()->ajax()) {
                return datatables()->of(ASDSale::with('sales', 'design', 'supervisor', 'source')->where('type', 2)->orderBy('id', 'desc')->limit(500)->get())
                ->addIndexColumn()
                ->make(true);
            }
        } else {
