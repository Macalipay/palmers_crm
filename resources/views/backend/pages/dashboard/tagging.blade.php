...
<main class="content">
    <div class="container-fluid">
        ...
        <div class="header">
            ...
            <p class="header-subtitle">Summary of Manual Tagging.</p>
        </div>
                ...
                <div class="row col-12">
                    <div class="col-12 col-xs-12 col-sm-12 col-lg-3 col-xl-3">
                        ...
                        <div class="poll-action">
                            <a href="{{url('poll_tag/survey-analyst/' . $poll_create->id)}}" class="btn btn-primary mb-2">Poll/Survey Analyst</a>
                            <a href="{{url('poll_tag/join/' . $poll_create->id)}}" class="btn btn-light">Manage Manual Tagging</a>
                        </div>
                    </div>
                                                ...
                                                @forelse ($poll_datas as $poll_data)
                                                    ...
                                                    {{-- @php ($percentage = $vote / $total_vote) --}}
                                                        <tr>
                                                            ...
                                                            <td class="text-right">{{ $count = App\PollVoteTag::where('poll_id', $poll_data->poll_id)->where('politician_id', $poll_data->politician_id)->count() }}</td>
                                                            <td class="d-xl-table-cell progress_value" total="{{$count_total}}">
                                                                <div class="progress">
                                                                    @if (App\PollVoteTag::where('poll_id', $poll_data->poll_id)->where('position_id', $poll_data->position_id)->groupBy('position_id', 'poll_id')->count() !== 0)
                                                                    <div class="progress-bar bg-primary-dark" role="progressbar" style="width: {{($count / App\PollVoteTag::where('poll_id', $poll_data->poll_id)->where('position_id', $poll_data->position_id)->groupBy('position_id', 'poll_id')->count()) * 100}}%">{{
                                                                        number_format(($count/App\PollVoteTag::where('poll_id', $poll_data->poll_id)->where('position_id', $poll_data->position_id)->groupBy('position_id', 'poll_id')->count()) * 100, 2)
                                                                    }}%</div>
                                                                    @endif
                                                                </div>
                                                            </td>
        ...
        $(function() {
            $('#datatables').DataTable({
                ...
                fnInfoCallback: function(oSettings, iStart, iEnd, iMax, iTotal, sPre) {
                    return 'Showing ' + iStart + ' to ' + iEnd + ' of ' + iTotal + 'entries. <b>Total Voters: ' + $('.progress_value').attr('total') + '</b>';
                }
            });
        });
    </script>
