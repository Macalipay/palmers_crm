...
function handleClick(event, chartContext, config) {
    ...
    $.post('/home/get_calls', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), status: label }).done((response) => {
        var el = '';
        var status = '';
        var lead = '';
...
function generateReport(date) {
    $.get('/home/get_record/' + date, function(response) {
        var division_html = '';
        var associate_html = '';
        var agent_html = '';
        var industry_html = '';
...
function getDaily() {
    $.post('/home/get_daily', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val() }).done((response) => {
        var el = '';
...
function getDivision(id) {
    $.post('/home/get_division', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), division_id: id }).done(function(response) {
...
function getBranch(id) {
    $.post('/home/get_branch', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), branch_id: id }).done((response) => {
        var el = '';
        if(response.sale.length !== 0) {
            $.each(response.sale, (i,v)=>{
                el += `<div class="sales-item">
...
function getAgent(id) {
    $.post('/home/get_agent', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), agent: id }).done((response) => {
        var el = '';
        if(response.sale.length !== 0) {
            $.each(response.sale, (i,v)=>{
                el += `<div class="sales-item">
...
function getAssociates(id) {
    $.post('/home/get_associate', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), associate: id }).done((response) => {
        var el = '';
        if(response.sale.length !== 0) {
            $.each(response.sale, (i,v)=>{
                el += `<div class="sales-item">
...
function getIndustry(detail) {
    $.post('/home/get_industry', { _token: $('meta[name="csrf-token"]').attr('content'), date: $('#filter_date').val(), industry: detail }).done((response) => {
        var el = '';
        $.each(response.sale, (i,v)=>{
